...
def RemoveChineseBooks(ke_dir):
    ...
    for root, dirs, files in list_book_dirs:
        for f in files:
            ...
            rel_path_bkfile = (
                bkfile.replace(books_dir, "").lstrip("/").lstrip("\\")
            )  # Relative path
            all_lines = []
            try:
...
def Main():
    ...
    custom_file = os.path.join(
        os.path.dirname(__file__), CUSTOM_FILE
    )  # file for saving your custom info
    ...
    for index, line in enumerate(slcfg):
        ...
        if mt:
            ...
            if domain.endswith("appspot.com") and domain not in (
                "http://%s.appspot.com" % app,
                "https://%s.appspot.com" % app,
            ):
