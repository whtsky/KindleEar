...
class BaseFeedBook:
    ...
    def isfiltered(self, url):
        if not self.url_filters:
            ...
            self.compiled_urlfilters = [
                re.compile(unicode(flt), re.I) for flt in self.url_filters
            ]
...
"""
        ...
        for feed in self.feeds:
            ...
            if result.status_code == 200 and result.content:
                ...
                for e in feed["entries"][: self.max_articles_per_feed]:
                    ...
                    if self.oldest_article > 0 and updated:
                        ...
                        if delta.days * 86400 + delta.seconds > threshold:
                            self.log.info(
                                ...
                                % (updated.strftime("%Y-%m-%d %H:%M:%S"), e.link)
                            )
                            continue
                    ...
                    urlfeed = urllib.quote_plus(
                        urlfeed.encode("utf-8"),
                        r"""~`!@#$%^&*()|\\/,.<>;:"'{}[]?=-_+""",
                    )
                    urls.append((section, title, urlfeed, desc))
            else:
                self.log.warn(
                    "fetch rss failed(%s):%s"
    ...
    def Items(self):
        ...
        readability = (
            self.readability
            if self.fulltext_by_readability
            else self.readability_by_soup
        )
        prevsection = ""
    ...
    def login(self, opener, decoder):
        ...
        self.log.info(
            "Form selected for login:name(%s),id(%s),class(%s)"
            % (form.get("name"), form.get("id"), form.get("class"))
        )

        method = form.get("method", "get").upper()
        action = (
            self.urljoin(self.login_url, form["action"])
            if form.get("action")
            else self.login_url
        )
        ...
        # 判断帐号域和密码域
        inputs = list(
            form.find_all("input", attrs={"type": ["text", "email", "password"]})
        )
        ...
        if not field_name or not field_pwd:
            self.log.warn(
                "Cant determine fields for account and password in login form!"
            )
            return
    ...
    def SelectLoginForm(self, soup):
        ...
        if isinstance(self.form_4_login, (int, long)):  # 通过序号选择表单
            ...
            if self.form_4_login.startswith("#"):  # id
                form = soup.find(
                    lambda f: f.name == "form" and f.get("id") == self.form_4_login[1:]
                )
            elif self.form_4_login.startswith("."):  # class
                form = soup.find(
                    lambda f: f.name == "form"
                    and self.form_4_login[1:] in f.get("class", [])
                )
            else:  # name & id & class
                form = soup.find(
                    lambda f: f.name == "form"
                    and (
                        f.get("name") == self.form_4_login
                        or f.get("id") == self.form_4_login
                        or self.form_4_login in f.get("class", [])
                    )
                )
        else:  # 自动猜测
            ...
            if len(forms) == 1:
                ...
                for f in forms:
                    ...
                    if (
                        len(
                            f.find_all(
                                lambda e: e.name == "input"
                                and e.get("type", "").lower() == "password"
                            )
                        )
                        == 1
                    ):
    ...
    def fetch(self, url, opener, decoder):
        ...
        if status_code not in (200, 206) or not content:
            self.log.warn(
                "fetch page failed(%s):%s." % (URLOpener.CodeMap(status_code), url)
            )
            return None

        # debug_mail(content)
        return self.AutoDecodeContent(
            content, decoder, self.page_encoding, opener.realurl, result.headers
        )
    ...
    def AutoDecodeContent(
        self, content, decoder, defaultEncoding=None, url=None, headers=None
    ):
        if defaultEncoding:
            try:
    ...
    def readability(self, article, url):
        ...
        try:
            ...
            if imgtype:  # 如果是图片，则使用一个简单的html做为容器
                ...
                tmpHtml = (
                    '<html><head><title>Picture</title></head><body><img src="%s" /></body></html>'
                    % fnimg
                )
        ...
        if self.keep_image:
            ...
            for img in soup.find_all("img"):
                ...
                imgcontent = (
                    self.process_image(imgresult.content)
        ...
        for img in soup.find_all("img"):
            ...
            if not imgUrl or imgUrl.endswith("/none.gif"):
                for attr in img.attrs:
                    if attr != "src" and (
                        ("src" in attr) or (attr == "data-original")
                    ):  # 很多网站使用data-src|data-original
                        imgUrl = img[attr]
                        break
                if not imgUrl:
                    for attr in img.attrs:
                        if attr != "src" and (
                            ("data" in attr) or ("file" in attr)
                        ):  # 如果上面的搜索找不到，再大胆一点猜测url
                            imgUrl = img[attr]
                            break
    ...
    def process_image(self, data):
        ...
        try:
            if (
                not opts
                or not opts.process_images
                or not opts.process_images_immediately
            ):
                return data
    ...
    def MakeShareLink(self, sharetype, user, url, soup):
        if sharetype in ("evernote", "wiz"):
            ...
            href = "%s/share?act=pocket&u=%s&h=%s&t=%s&url=" % (
                DOMAIN,
                user.name,
                (hashlib.md5(user.pocket_acc_token_hash or "").hexdigest()),
            ...
            href = ""
        if user.share_fuckgfw and sharetype in (
            "evernote",
            "wiz",
            "facebook",
            "twitter",
    ...
    def Items(self):
        ...
        for section, url in self.feeds:
            ...
            if self.keep_image:
                ...
                for img in soup.find_all("img"):
                    ...
                    if imgcontent:
                        if isinstance(imgcontent, list):  # 一个图片分隔为多个图片
                            ...
                            if (
                                len(imgcontent) < self.img_min_size
                            ):  # rexdf too small image
                                img.decompose()
                                continue
                ...
                # 去掉图像上面的链接
                if self.user and self.user.remove_hyperlinks in (
                    u"image",
                    u"all",
                    "image",
                    "all",
            ...
            # 提取文章内容的前面一部分做为摘要，[漫画模式不需要摘要]
            brief = u""
            if GENERATE_TOC_DESC and (
                (not self.user) or self.user.book_mode != "comic"
            ):
                for h in body.find_all(["h1", "h2"]):  # 去掉h1/h2，避免和标题重复
                    h.decompose()
                for s in body.stripped_strings:
...
class BaseComicBook(BaseFeedBook):
    ...
    def ParseFeedUrls(self):
        ...
        for item in self.feeds:
            ...
            if next_chapter_index < len(chapterList):
                ...
                if not imgList:
                    self.log.warn("can not found image list: %s" % chapter_url)
                    break
                for img in imgList:
                    page_count += 1
                ...
                self.UpdateLastDelivered(
                    bookname, chapter_title, next_chapter_index + 1
                )
            else:
                self.log.info(
    ...
    def Items(self):
        ...
        for section, fTitle, url, desc in urls:
            ...
            if content == None:
                self.log.warn(
                    "Image adjust error, try again: {}".format(url.encode("utf-8"))
                )
                content = self.adjustImgContent(content)
                if content == None:
            ...
            if imgType:
                ...
                if not imgContentList:
                    ...
                    if not any(isComics):
                        ...
                        imgContentList = [
                            item
                            for idx, item in enumerate(imgContentList)
                            if isComics[idx]
                        ]
