...
class Url2Book(BaseHandler):
    ...
    def GET(self):
        ...
        if booktype == "Download":  # 直接下载电子书并推送
            ...
            for url in urls.split("|"):
                ...
                if content:
                    ...
                    self.deliverlog(
                        username, str(to), filename, 0, status=dlinfo, tz=tz
                    )
        ...
        if len(book.feeds) > 1:
            ...
            setMetaData(
                oeb, subject, language, local_time(tz=tz), pubtype="book:book:KindleEar"
            )
            ...
            if sec_or_media.startswith(r"image/"):
                ...
                if len(book.feeds) > 1:
                    ...
                    item = oeb.manifest.add(
                        id_, href, "application/xhtml+xml", data=content
                    )
        ...
        if itemcnt > 0:
            ...
            self.SendToKindle(
                username, to, book.title, booktype, str(oIO.getvalue()), tz
            )
            ...
            self.deliverlog(
                username, str(to), book.title, 0, status="fetch failed", tz=tz
            )
