...
class Worker(BaseHandler):
    ...
    def GET(self):
        ...
        if (";" in to) or ("," in to):
            ...
            to = list(
                filter(lambda x: x.find("@", 1, len(x) - 1) > 0, to)
            )  # 最简单的判断是否是EMAIL
        ...
        bookTitle = (
            "%s %s" % (book4meta.title, local_time(titlefmt, tz))
            if titlefmt
            else book4meta.title
        )
        ...
        if coverfile:
                ...
                try:
                    ...
                    if imgData:
                        ...
                        if imgType:  # 如果是合法图片
                            ...
                            main.log.warn(
                                u"content of cover is invalid : [%s]." % bookTitle
                            )
                            imgData = None
                except:
                    main.log.exception(
            ...
            item = oeb.manifest.add(
                id_, href, "image/jpeg", data=self.MergeCovers(bks, opts, user)
            )
            oeb.guide.add("cover", "Cover", href)
            oeb.metadata.add("cover", id_)
        ...
        for cv in coverfiles:
            ...
            if cv == DEFAULT_COVER and user.cover:
                ...
                try:
                    ...
                    main.log.warn(
                        "[MergeCovers] Content of cover is invalid : [%s]." % str(e)
                    )
                    cv = DEFAULT_COVER
                    img = None
            try:
                ...
                main.log.warn(
                    "[MergeCovers] Cover file invalid [%s], %s" % (str(cv), str(e))
                )
            else:
