...
class Share(BaseHandler):
    ...
    def SaveToEvernoteWiz(self, user, action, orgUrl):
        ...
        for sec_or_media, url, title, content, brief, thumbnail in book.Items():
            if sec_or_media.startswith(r"image/"):
                if self.SHARE_IMAGE_EMBEDDED:
                    attachments.append(
                        mail.Attachment(title, content, content_id="<%s>" % title)
                    )
                else:
        ...
        if html:
            self.SendHtmlMail(
                user.name, to, title, html.encode("utf-8"), attachments, user.timezone
            )
            info = _("'%(title)s'<br/><br/>Saved to %(act)s [%(email)s] success.") % (
                {"title": title, "act": action, "email": hide_email(to)}
            )
            ...
            self.deliverlog(
                user.name, str(to), title, 0, status="fetch failed", tz=user.timezone
            )
    ...
    def SaveToPocket(self, user, action, orgUrl):
        ...
        if not user.pocket_access_token:
            info = T_INFO % (
                "Pocket unauthorized",
                "Unauthorized Pocket!<br/>Please authorize your KindleEar application firstly.",
            )
            return info.encode("utf-8")
        ...
        if hashlib.md5(user.pocket_acc_token_hash).hexdigest() != tkHash:
            info = T_INFO % (
                "Action rejected",
                "Hash not match!<br/>KindleEar refuse to execute your command.",
            )
            return info.encode("utf-8")
        ...
        try:
            ...
            info = T_INFO % (
                "Failed to save",
                _("Failed save to Pocket.<br/>") + str(e),
            )
        else:
    ...
    def SaveToInstapaper(self, user, action, orgUrl):
        ...
        if not user.instapaper_username or not user.instapaper_password:
            info = T_INFO % (
                "No authorize info",
                "Instapaper username and password have to provided fistly!<br/>Please fill them in your KindleEar application.",
            )
            return info.encode("utf-8")
        ...
        if user.instapaper_username != name:
            info = T_INFO % (
                "Action rejected",
                "Username not match!<br/>KindleEar refuse to execute your command.",
            )
            return info.encode("utf-8")
        ...
        if ret.status_code in (200, 201):
            ...
            info += (
                '<br/><p style="text-align:right;color:red;">by KindleEar &nbsp;</p>'
            )
            info = T_INFO % ("Saved to Instapaper", info)
        elif ret.status_code == 403:
            info = (
                _(
                    "Failed save to Instapaper<br/>'%s'<br/><br/>Reason : Invalid username or password."
                )
                % title
            )
            info += (
            ...
            info = _(
                "Failed save to Instapaper<br/>'%s'<br/><br/>Reason : Unknown(%d)."
