...
class SharedLibrary(BaseHandler):
    ...
    def GET(self):
        ...
        url = urlparse.urljoin(
            "http://kindleear.appspot.com/", SharedLibrarykindleearAppspotCom.__url__
        )
    ...
    def POST(self):
        ...
        if result.status_code == 200 and result.content:
            ...
            return json.dumps(
                {
                    "status": _(
                        "Cannot submit data to kindleear.appspot.com, status: %s"
                        % URLOpener.CodeMap(result.status_code)
                    )
                }
            )
            ...
            shared_data.append(
                {
                    ...
                    "s": d.subscribed,
                    "d": int(
                        (d.created_time - datetime.datetime(1970, 1, 1)).total_seconds()
                    ),
                }
            )
    ...
    def POST(self):
        ...
        if dbItem:
            ...
            SharedRss(
                ...
                subscribed=1,
                created_time=now,
                invalid_report_days=0,
                last_invalid_report_time=now,
            ).put()
...
class SharedLibraryCategorykindleearAppspotCom(BaseHandler):
    ...
    def GET(self):
        ...
        web.header("Content-Type", "application/json")
        return json.dumps(
            [item.name for item in SharedRssCategory.all().order("-last_updated")]
        )
