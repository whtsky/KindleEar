...
class Admin(BaseHandler):
    ...
    def GET(self):
        ...
        return self.render(
            "admin.html", "Admin", current="admin", user=user, users=users
        )
        ...
        if all((op, p1, p2)):  # 修改当前登陆账号的密码
            ...
            if user.name != "admin":
                ...
                try:
                    ...
                    myfeeds = Book(
                        title=MY_FEEDS_TITLE,
                        description=MY_FEEDS_DESC,
                        builtin=False,
                        keep_image=True,
                        oldest_article=7,
                    ...
                    if expiration:
                        au.expires = datetime.datetime.utcnow() + datetime.timedelta(
                            days=expiration
                        )
...
class AdminMgrPwd(BaseHandler):
    ...
    def GET(self, name):
        ...
        return self.render(
            "adminmgrpwd.html", "Change password", tips=tips, username=name
        )
...
class DelAccount(BaseHandler):
    ...
    def GET(self, name):
        ...
        if main.session.username == "admin" or (name and name == main.session.username):
            ...
            return self.render(
                "delaccount.html", "Delete account", tips=tips, username=name
            )
        else:
            raise web.seeother(r"/")
