...
class Mylogs(BaseHandler):
    ...
    def GET(self):
        ...
        try:
            ...
            mylogs = sorted(
                DeliverLog.all().filter("username = ", user.name),
                key=attrgetter("time"),
                reverse=True,
            )[:100]
        ...
        if user.name == "admin":
            try:
                ...
                lastDelivered = sorted(
                    LastDelivered.all().fetch(),
            ...
            try:
                ...
                lastDelivered = sorted(
                    LastDelivered.all().filter("username = ", user.name),
                    key=attrgetter("datetime"),
                    reverse=True,
                )[:100]
    ...
    def POST(self, mgrType):
        ...
        if mgrType.lower() == "delete":
            ...
            if dbItem:
                ...
                return json.dumps(
                    {"status": _("The LastDelivered item(%d) not exist!") % id_}
                )
