...
class AdvArchive(BaseHandler):
    ...
    def GET(self):
        ...
        return self.render(
            ...
            savetoinstapaper=SAVE_TO_INSTAPAPER,
    ...
    def POST(self):
        ...
        if "importfile" in x:
            ...
            for o in self.walkOutline(rsslist):
                title, url, isfulltext = (
                    o.text,
                    urllib.unquote_plus(o.xmlUrl),
                    o.isFulltext,
                )  # isFulltext为非标准属性
...
%s
        ...
        for feed in Feed.all().filter("book = ", user.ownfeeds):
            outlines.append(
                ...
                % (
                    feed.title,
                    urllib.quote_plus(feed.url.encode("utf-8")),
                    feed.isfulltext,
                )
            )
        outlines = "\n".join(outlines)
        ...
        web.header(
            "Content-Disposition", "attachment;filename=KindleEar_subscription.xml"
        )
...
class AdvUploadCoverImage(BaseHandler):
    ...
    def GET(self, tips=None):
        ...
        return self.render(
            ...
            current="advsetting",
            user=user,
            advcurr="uploadcoverimage",
            formaction=AdvUploadCoverImageAjax.__url__,
            deletecoverhref=AdvDeleteCoverImageAjax.__url__,
            tips=tips,
        )
        ...
        try:
            ...
            if user and file_:
                ...
                if (width > self.MAX_IMAGE_PIXEL) or (height > self.MAX_IMAGE_PIXEL):
                    ratio = min(
                        float(self.MAX_IMAGE_PIXEL) / float(width),
    ...
    def GET(self, authType):
        ...
        try:
            ...
            return self.render(
                ...
                tips=_("Authorization Error!<br/>%s") % str(e),
            )
    ...
    def GET(self, authType):
        ...
        try:
            ...
            return self.render(
                ...
                urltoback="/advarchive",
                tips=_(
                    "Failed to request authorization of Pocket!<hr/>See details below:<br/><br/>%s"
                )
                % str(e),
            )
...
class VerifyAjax(BaseHandler):
    ...
    def POST(self, verType):
        ...
        if ret.status_code in (200, 201):
            ...
            respDict["status"] = _(
                "The Instapaper service encountered an error. Please try again later."
            )
