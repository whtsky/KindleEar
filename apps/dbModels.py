...
class Book(db.Model):
    ...
    def feeds(self):
        try:
            ...
            return sorted(
                Feed.all().filter("book = ", self.key()), key=attrgetter("time")
            )
...
class KeUser(db.Model):  # kindleEar User
    ...
    book_mode = (
        db.StringProperty()
    )  # added 2017-08-31 书籍模式，'periodical'|'comic'，漫画模式可以直接全屏
    expiration_days = db.IntegerProperty()  # added 2018-01-07 账号超期设置值，0为永久有效
    remove_hyperlinks = (
        db.StringProperty()
    )  # added 2018-05-02 去掉文本或图片上的超链接{'' | 'image' | 'text' | 'all'}
    ...
    def subscription_info(self, title):
        return (
            SubscriptionInfo.all()
            .filter("user = ", self.key())
            .filter("title = ", title)
            .get()
        )
...
class SharedRss(db.Model):
    ...
    invalid_report_days = db.IntegerProperty(
        default=0
    )  # some one reported it is a invalid link
    last_invalid_report_time = (
        db.DateTimeProperty()
    )  # a rss will be deleted after some days of reported_invalid
    ...
    def categories(self):
        return [
            item.category
            for item in db.GqlQuery("SELECT DISTINCT category FROM SharedRss")
        ]
