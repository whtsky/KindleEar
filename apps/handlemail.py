...
class HandleMail(InboundMailHandler):
    def receive(self, message):
        ...
        if not links and not forceToArticle:  # 如果通过正常字符（显示出来的）判断没有链接，则看html的a标签
            ...
            # 如果字数太多，则认为直接推送正文内容
            if not forceToLinks and (
                len(links) != 1 or len(text) > WORDCNT_THRESHOLD_FOR_APMAIL
            ):
                links = []
        ...
        if links:
            ...
            if hasimage or (user.book_type == "epub"):
                ...
                item = oeb.manifest.add(
                    id_, href, "application/xhtml+xml", data=unicode(soup)
                )
                    ...
                    for filename, content in message.attachments:
                        ...
                        if mimetype:
                            try:
                                ...
                                item = oeb.manifest.add(
                                    id_, href, mimetype, data=content
                                )
