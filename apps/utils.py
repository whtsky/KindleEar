...
def InsertToc(oeb, sections, toc_thumbnails, insertHtmlToc=True, insertThumbnail=True):
    ...
    for sec in sections.keys():
        css = [
            ".pagebreak{page-break-before:always;}h1{font-size:2.0em;}h2{font-size:1.5em;}h3{font-size:1.4em;}h4{font-size:1.2em;}h5{font-size:1.1em;}h6{font-size:1.0em;}"
        ]
        html_content = []
        secondary_toc_list = []
        first_flag = False
        ...
        html_content.insert(
            0,
            '<html><head><title>%s</title><style type="text/css">%s</style></head><body>'
            % (sec, "".join(css)),
        )
        ...
        if insertHtmlToc:
            html_toc_ = [
                "<html><head><title>toc</title></head><body><h2>%s</h2><ol>"
                % (sec_with_num)
            ]
        for title, anchor, brief, thumbnail in secondary_toc_list:
            if insertHtmlToc:
                html_toc_.append(
                    '&nbsp;&nbsp;&nbsp;&nbsp;<li><a href="%s#%d">%s</a></li><br />'
                    % (href, anchor, title)
                )
            ncx_toc.append(
                ("article", title, "%s#%d" % (href, anchor), brief, thumbnail)
    ...
    if insertHtmlToc:
        ...
        html_toc_1 = [
            u"<html><head><title>Table Of Contents</title></head><body><h2>%s</h2><ul>"
            % (TABLE_OF_CONTENTS)
        ]
        html_toc_1_ = []
        ...
        for a in xrange(len(html_toc_2) - 1, -1, -1):
            ...
            item = oeb.manifest.add(
                id_, href, "application/xhtml+xml", data=" ".join(html_toc_2[a])
            )
            oeb.spine.insert(0, item, True)
            html_toc_1_.append(
                '&nbsp;&nbsp;&nbsp;&nbsp;<li><a href="%s">%s</a></li><br />'
                % (href, name_section_list[a])
            )
        html_toc_2 = []
    ...
    for ncx in ncx_toc:
        ...
        if ncx[0] == "section":
            ...
            sectoc.add(
                unicode(ncx[1]),
                ncx[2],
                description=ncx[3] if ncx[3] else None,
                klass="article",
                play_order=po,
                id="article-%d" % po,
                toc_thumbnail=toc_thumbnail,
