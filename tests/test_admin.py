...
class AdminTestCase(BaseTestCase):
    ...
    def setUp(self):
        super().setUp()
        #测试数据库跨次运行会保留数据，先清理账号，保证用例可重复运行
        KeUser.delete().where(KeUser.name != 'admin').execute()
        user = KeUser.get_or_none(KeUser.name == 'admin') #恢复管理员密码，防止上次运行中断遗留
        if user:
            user.passwd_hash = user.hash_text('admin')
            user.save()
    ...
    def test_add_account_and_delete(self):
        ...
        self.assertEqual(user.cfg('email'), '1@1.com')
    ...
    def test_change_admin_password(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn('Edit account', resp.text)
        ...
        data['username'] = 'admin'
        data['orgpwd'] = 'admin1'
        data['password2'] = '1' #新旧密码检查在格式检查之后，先保证两个新密码一致
        resp = self.client.post('/account/change/admin', data=data)
        self.assertEqual(resp.status_code, 200)
